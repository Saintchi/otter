...
package com.alibaba.otter.common.push.supplier.ads;
 ...
 * @version 4.1.8
 */
public class AdsDatasourceSupplier extends AbstractDatasourceSupplier {

    private static final Logger            log          = LoggerFactory.getLogger(AdsDatasourceSupplier.class);
    ...
    // hyunji 原来是groupKey
    private static final String DATAID_ADS = "dataid_ads";

    private AdsDatasourceSupplier(){
    }

    public static AdsDatasourceSupplier newInstance() {
        AdsDatasourceSupplier supplier = new AdsDatasourceSupplier();
        return supplier;
    }
    ...
    public void doStart() {
        subscribeCallback = new SubscribeCallback() {
            ...
            public void callback(String matrixInfo) {
                ...
                synchronized (AdsDatasourceSupplier.this.lock) {
                    AdsDatasourceSupplier.this.haInfo = parse(matrixInfo);
                    AdsDatasourceSupplier.this.callback();
                }
            }
        };
        this.init();
    ...
    @Override
    public void doStop() {
        this.mediaSubscribeManager.unRegisterCallback(DATAID_ADS, subscribeCallback);
    ...
    public synchronized void init() {
        ...
        String matrixStr = mediaSubscribeManager.fetchConfig(DATAID_ADS);
        this.haInfo = parse(matrixStr);
        this.mediaSubscribeManager.registerCallback(DATAID_ADS, subscribeCallback);
    }
    ...
    private void callback() {
        ...
        for (DatasourceChangeCallback callback : callbacks) {
            callback.masterChanged(AdsDatasourceSupplier.this.fetchMaster());
        }
